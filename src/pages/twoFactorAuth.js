...
const TwoFactorAuth = () => {
    const { error, successAuth, gotEmail, user, twoFactorAuthToken, loading } = useSelector(getLoginUser);
    ...
    return(twoFactorAuthToken &&
        <div data-testid='two-factor-auth' className="two-factor-container">
            <PageDesign/>
            {error && <SuccessFailPopup handleClick={handleClick} message={error} success={successAuth} statusMessage="Failed!"/>}
            {!error && <FormContainerTwoFact formatTime={formatTime(seconds)} gotEmail={gotEmail} onLinkClick={onLinkClick } onClick={onClick}/>}
        </div>
    );
}
