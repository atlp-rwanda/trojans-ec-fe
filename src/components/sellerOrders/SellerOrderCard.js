...
const SellerOrderCard = ({ status, product, onAccept, onDecline, onDeliver }) => {
  const loading = false;
    ...
    <div className="max-w-[100%] hover:scale-[1.02] hover:cursor-pointer h-96 mb-3 rounded-lg transition-all duration-200 ease-linear border shadow-md bg-whiteColor border-purple-100 flex flex-col justify-between items-center">
      ...
      <div className="px-3 py-2 flex flex-col justify-around items-center w-full h-[32%]">
        ...
        <div className="flex text-sm justify-around items-center w-full">
          <button className="px-3 py-1 rounded-sm bg-yellow-900 text-white hover:bg-yellow-700 transition-all duration-200 ease-linear hover:cursor-pointer" onClick={()=>onAccept()}>
            Approve
          </button>
          <button className="px-3 py-1 rounded-sm bg-red-900 text-white hover:bg-red-700 transition-all duration-200 ease-linear hover:cursor-pointer" onClick={()=>onDecline()}>
            Decline
          </button>
          <button className="px-3 py-1 rounded-sm bg-green-900 text-white hover:bg-green-700 transition-all duration-200 ease-linear hover:cursor-pointer" onClick={()=>onDeliver()}>
            Delivered
          </button>
        </div>
      </div>
