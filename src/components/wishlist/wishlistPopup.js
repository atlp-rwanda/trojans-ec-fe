...
const WishlistPopup = ({ message, handleClick, success, statusMessage }) => {
  return (
    <div className="success-fail-popup translate-x-0 translate-y-[-50%]">
      {success && (
        <>
      ...
      <p>{message}</p>
      <button className="button two-fact-btn" onClick={handleClick}>
        ok
      </button>
    </div>
  );
