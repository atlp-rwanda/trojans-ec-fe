    ...
    <>
        {
            currentUsers.map((user, index)=>(
                <tr data-testid='tb-row' key={index} className={`${index % 2 != 0 ? `bg-gray-100` : `bg-white`}`}>
                    ...
                    <td className='p-3 text-md tracking-wide text-center'>{user.gender}</td>
                    <td className='p-3 text-md tracking-wide text-center'>{user.preferredLanguage}</td>
                    <td className='p-3 text-md tracking-wide text-center'>{new Date(user.birthdate).toLocaleDateString()}</td>
                    <td className='p-3 text-md tracking-wide text-center'>{user.status}</td>
                        ...
                        <div className="drop-content absolute right-0 w-48 py-2 mt-2 bg-white bg-gray-100 rounded-md shadow-xl">
                            {
                                dropOption.map((options, id) => (
                                    <a 
                                        ...
                                        onClick={()=>handleRole(user.id, options)}
                                    >
                                    Make {options}
                                    </a>
