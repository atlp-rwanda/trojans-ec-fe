...
mockAxios.put.mockResolvedValue({
  data: { status: 200, message: "Product updated", data: updatedProduct },
});
...
test("testing updating a product", async () => {
  ...
  await waitFor(() =>
    expect(screen.getByTestId("update-product-form")).toBeInTheDocument()
  );
  ...
  expect(
    await screen.findByText(/Quantity must be a number/i)
  ).toBeInTheDocument();
