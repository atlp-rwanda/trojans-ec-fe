...
describe("testing TwoFactorAuth component", () => {
  test("login as seller and navigate 2fa page", async () => {
    mockAxios.post.mockResolvedValue({
      data: { status: 200, role: "admin", token: "some token string" },
    });
  ...
  test("clicking on the try again link and navigate to login page", async () => {
    mockAxios.post.mockResolvedValue({
      data: { status: 200, role: "seller", token: "some token string" },
    });
  ...
  describe("renders error messages", () => {
    test("render isRequired and length of five errors", () => {
      ...
      const errorElmTwo = screen.getByText(
        /Verification code must be exactly 5 numbers/i
      );
    ...
    test("render first digit must be zero error", () => {
      ...
      const errorElmOne = screen.getByText(
        /The first digit must be greater than 0/i
      );
      expect(errorElmOne).toBeInTheDocument();
    });
  });
  ...
  test("render loader", async () => {
    mockAxios.post.mockRejectedValue({
      response: { data: { status: 400, message: "invalid otp" } },
    });
    ...
    const buttonElmOne = screen.getByRole("button", { name: "Verify" });
    act(() => fireEvent.submit(buttonElmOne));
    const loaderElm = screen.getByTestId("loader-2fa");
    expect(loaderElm).toBeInTheDocument();
    await waitFor(() =>
      expect(screen.getByText(/Wrong verification code!/i)).toBeInTheDocument()
    );
  });
  ...
  test("500 response from the server", async () => {
    mockAxios.post.mockRejectedValue({
      response: {
        data: { status: 500, message: "server error or otp expired" },
      },
    });
    localStorage.setItem("userAuth", JSON.stringify("some token to test"));
    ...
    await waitFor(() =>
      expect(
        screen.getByText(
          /An error occurred while trying to verify your account. Please try again later./i
        )
      ).toBeInTheDocument()
    );
  });

  test.skip("testing otp send request", async () => {
    mockAxios.post.mockResolvedValue({
      data: { status: 200, role: "seller", token: userToken },
    });
    localStorage.setItem("userAuth", JSON.stringify("some token to test"));
