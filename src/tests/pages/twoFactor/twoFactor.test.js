...
describe('testing TwoFactorAuth component', ()=>{
    test('login as seller and navigate 2fa page', async ()=>{
        mockAxios.post.mockResolvedValue({ data: { status: 200, role: "admin", token: "some token string" } });
    ...
    test('clicking on the try again link and navigate to login page', async ()=>{
        mockAxios.post.mockResolvedValue({ data: { status: 200, role: "seller", token: "some token string" } });
    ...
    test('render loader', async () => {
        ...
        await waitFor(() => expect(screen.getByText(/Wrong verification code!/i)).toBeInTheDocument());
    })
    ...
    test('render fail message if no two factor token', async ()=>{
        ...
        await waitFor(() => expect(screen.getByText(/Please login first!/i)).toBeInTheDocument());
    })
    ...
    test('500 response from the server', async ()=>{
        ...
        await waitFor(() => expect(screen.getByText(/An error occurred while trying to verify your account. Please try again later./i)).toBeInTheDocument());
    })

    test.skip('testing otp send request', async ()=>{
        mockAxios.post.mockResolvedValue({ data: { status: 200, role: "seller", token: userToken } });
        localStorage.setItem("userAuth", JSON.stringify("some token to test"));
