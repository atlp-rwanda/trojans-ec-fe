...
import 'setimmediate';
import { render, screen, fireEvent, waitFor, waitForElementToBeRemoved } from '../jest.setup.js';
...
describe('testing update form', () => {
    test('should render the form with input fields', async() => {
        ...
        expect(await screen.findByText(/country is a required field/i)).toBeInTheDocument();
        expect(await screen.findByText(/city is a required field/i)).toBeInTheDocument();
        expect(await screen.findByText(/province is a required field/i)).toBeInTheDocument();
        expect(await screen.findByText(/street is a required field/i)).toBeInTheDocument();
        expect(await screen.findByText(/postalCode is a required field/i)).toBeInTheDocument();
        expect(await screen.findByText(/preferredCurrency is a required field/i)).toBeInTheDocument();
        expect(await screen.findByText(/preferredLanguage is a required field/i)).toBeInTheDocument();
    });
        ...
        test('testig profile update functionalities', async() => {
            ...
            await waitFor(()=> expect(screen.queryByTestId("profile")).toBeInTheDocument());
            const button = screen.getByRole('button', {name: /UPDATE/i})
            act(() => fireEvent.click(button));
            await waitFor(()=> expect(screen.getByTestId('updateLoader')).toBeInTheDocument());
