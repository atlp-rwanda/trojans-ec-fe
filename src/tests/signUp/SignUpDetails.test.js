...
describe("Details component", () => {
  ...
  test("form should show error when inputs are empty", async () => {
    ...
    const cpasswordError = await screen.findByText(
      "Comfirm password is a required field"
    );
    const genderError = await screen.findByText(/gender is a required field/i);
    const dobError = await screen.findByText(
      /Date of birth is a required fiel/i
    );
    const languageError = await screen.findByText(
      /languange is a required field/i
    );
    const currencyError = await screen.findByText(
      /currency is a required field/i
    );
...
test("testing registration functionalities", async () => {
  mockAxiosInstance.post.mockResolvedValue({
    data: { status: 201, message: "success" },
  });
  act(() =>
    render(<SignUp />, {
      preloadedState: {
  ...
  const postalCodeError = await screen.findByText(
